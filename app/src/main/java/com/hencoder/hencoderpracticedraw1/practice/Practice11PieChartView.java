...
//        canvas.drawArc(300,200,600,500,88,92,true,paint);
    ...
    private void drawLineAndTxt(Canvas canvas) {
        updateDimen(rectF);
        canvas.translate(centerX, centerY);//移动画布原点坐标
    ...
    private void updateDimen(RectF rect) {
    ...
    private void drawLine(Canvas canvas, int start, int angles, String txt, int color, boolean isLast) {
        ...
        // 绘制字体
        if (stopX > 0) {
            paint.setTextAlign(Paint.Align.LEFT);//做点为起点
            canvas.drawText(txt, stopX + OUTER_LINE_LENGTH + 10, stopY - (rect.top - rect.bottom) / 2, paint);
        } else {
            paint.setTextAlign(Paint.Align.RIGHT);//右点为坐标点
            canvas.drawText(txt, stopX - OUTER_LINE_LENGTH - 10, stopY - (rect.top - rect.bottom) / 2, paint);
        }
